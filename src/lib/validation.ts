...
export const doctorProfileSchema = z.object({
  ...
  careerStartYear: z.coerce.number().int().min(1950).max(new Date().getFullYear()).optional().nullable(),
  education: z.array(z.string().trim().max(240)).max(12).optional().default([]),
  courses: z.array(z.string().trim().max(240)).max(20).optional().default([]),
  workHistory: z.array(z.string().trim().max(240)).max(15).optional().default([]),
  expertise: z.array(z.string().trim().max(200)).max(25).optional().default([]),
