...
export async function getCentersForService(serviceSlug: string, take?: number) {
  ...
  // ilk 12-yə düşməyəndə səhifə yalnız qiymətsizləri göstərirdi. İndi HAMISI
  // qaytarılır (getApprovedCenters-in default 60 limiti də aşılır), kəsimi
  // çağıran tərəf SORTDAN SONRA edir.
  return getApprovedCenters({ service: serviceSlug, take: take ?? 5000 });
}
