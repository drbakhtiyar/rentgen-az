                          ...
                          >
                            <div className="flex items-center justify-between">
                              <span className="truncate text-[11px] font-semibold text-slate-500">
                                {b.reason || t.block.closed}
                              </span>
                              {b.fixed ? (
                                <span className="text-[10px] text-slate-400">{t.block.fixed}</span>
                              ) : (
...
function RescheduleModal({
  ...
  return (
    <ModalShell title={t.resched.title} onClose={cancel}>
      ...
      <p className="mt-3 text-sm font-semibold text-ink-900">{t.resched.question}</p>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        >
          {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : null} {t.resched.yes}
        </button>
        <button
          type="button"
          disabled={busy}
...
function DeleteBlockButton({ id }: { id: string }) {
  ...
  return (
    <button
      ...
      onClick={async (e) => {
        e.stopPropagation();
        if (!confirm(t.block.delConfirm)) return;
        setBusy(true);
...
function AppointmentModal({
  ...
  async function onDelete() {
    if (!appt || !confirm(t.forms.delConfirm)) return;
    setBusy(true);
    setError(null);
  ...
  return (
    <ModalShell title={edit ? t.forms.editTitle : t.forms.newTitle} onClose={onClose}>
      <form onSubmit={onSubmit} className="space-y-3">
        <div className="grid grid-cols-2 gap-3">
          <div className="col-span-2">
            <label className="mb-1 block text-xs font-semibold text-slate-500">{t.forms.nameLabel}</label>
            <input name="name" required defaultValue={appt?.name ?? ""} className={fieldCls} placeholder={t.forms.namePh} />
          </div>
          <div className="col-span-2">
            <label className="mb-1 block text-xs font-semibold text-slate-500">{t.forms.phoneLabel}</label>
            <input name="phone" required defaultValue={appt?.phone ?? ""} className={fieldCls} placeholder={t.forms.phonePh} />
          </div>
          <div className="col-span-2">
          ...
          <div className="col-span-2">
            ...
            <input name="note" defaultValue={appt?.note ?? ""} className={fieldCls} placeholder={t.forms.notePh} />
          </div>
        </div>
        ...
        <div className="flex items-center justify-between gap-2 pt-1">
          <div className="flex gap-2">
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-full bg-brand-600 px-5 py-2 text-sm font-semibold text-white hover:bg-brand-700 disabled:opacity-50"
            >
              {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4" />} {t.common.save}
            </button>
            {edit && appt!.patientId && (
              <Link
              ...
              >
                {t.forms.card} <ExternalLink className="h-3.5 w-3.5" />
              </Link>
            )}
          </div>
          {edit && (
            ...
            >
              <Trash2 className="h-4 w-4" /> {t.common.del}
            </button>
          )}
        </div>
      </form>
...
function TimeBlockModal({ defaultYmd, onClose }: { defaultYmd: string; onClose: () => void }) {
  ...
  return (
    <ModalShell title={t.block.title} onClose={onClose}>
      <form onSubmit={onSubmit} className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="mb-1 block text-xs font-semibold text-slate-500">{t.block.start}</label>
            <input name="startTime" type="time" required className={fieldCls} />
          </div>
          <div>
            <label className="mb-1 block text-xs font-semibold text-slate-500">{t.block.end}</label>
            <input name="endTime" type="time" required className={fieldCls} />
          </div>
        </div>
        <div>
          <label className="mb-1 block text-xs font-semibold text-slate-500">{t.block.reasonLabel}</label>
          <input name="reason" className={fieldCls} placeholder={t.block.reasonPh} />
        </div>
        {error && <p className="text-sm font-medium text-red-600">{error}</p>}
        <button
          type="submit"
          ...
          className="inline-flex items-center gap-1.5 rounded-full bg-brand-600 px-5 py-2 text-sm font-semibold text-white hover:bg-brand-700 disabled:opacity-50"
        >
          {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Ban className="h-4 w-4" />} {t.block.submit}
        </button>
      </form>
    </ModalShell>
  );
