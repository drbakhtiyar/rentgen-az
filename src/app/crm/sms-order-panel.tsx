...
export function SmsOrderPanel({
  ...
  async function buy(pack: Pack) {
    if (!confirm(`${pack.qty}${t.smsBuy.confirmPre}${pack.price}${t.smsBuy.confirmPost}`)) return;
    setBusy(pack.qty);
    setError(null);
    setDone(null);
    ...
    setDone(`${pack.qty}${t.smsBuy.donePost}`);
    router.refresh();
  }
    ...
    <div>
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2 text-sm">
        <span className="inline-flex items-center gap-1.5 text-slate-600">
          ...
          {t.smsBuy.balanceLabel} <span className="font-semibold text-ink-900">{(walletBalanceMinor / 100).toFixed(2)} ₼</span>
        </span>
      ...
      {stockLimited && (
        <p className="mb-3 flex items-start gap-1.5 rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-800">
          ...
          {t.smsBuy.stockPre}<span className="font-semibold">{maxBuyable}</span>{t.smsBuy.stockPost}
        </p>
      )}
        ...
        {packages.map((p) => {
          ...
          return (
            <div key={p.qty} className={`rounded-xl border bg-white p-4 text-center ${outOfStock ? "border-slate-100 opacity-50" : "border-slate-200"}`}>
              ...
              <p className="text-xs text-slate-500">{t.smsBuy.smsWord}</p>
              ...
              >
                ...
                {outOfStock ? t.smsBuy.outOfStock : t.smsBuy.buy}
              </button>
              {cantAfford && !outOfStock && (
                <p className="mt-1.5 text-[10px] text-amber-600">{t.smsBuy.noMoney}</p>
              )}
            </div>
          );
        })}
