...
import {
  startAddDoctorAssistantAction,
  confirmAddDoctorAssistantAction,
  setDoctorAssistantActiveAction,
  removeDoctorAssistantAction,
} from "./actions";
...
/** Doctor-side twin of the CRM AssistantsManager (max 1 assistant). */
export function DoctorAssistantsManager({ initial }: { initial: Assistant[] }) {
  ...
  async function sendCode(e: React.FormEvent) {
    ...
    setBusy(true);
    setError(null);
    const res = await startAddDoctorAssistantAction({ firstName: first, lastName: last, phone });
    setBusy(false);
  ...
  async function confirmAdd(e: React.FormEvent) {
    ...
    setBusy(true);
    setError(null);
    const res = await confirmAddDoctorAssistantAction({ firstName: first, lastName: last, phone, code });
    setBusy(false);
    if (!res.ok) return setError(res.error ?? "Xəta");
    setStep("list");
    setFirst("");
  ...
  async function toggle(a: Assistant) {
    setBusy(true);
    await setDoctorAssistantActiveAction(a.id, !a.active);
    setBusy(false);
    router.refresh();
  }
  ...
  async function remove(a: Assistant) {
    ...
    setBusy(true);
    await removeDoctorAssistantAction(a.id);
    setBusy(false);
    router.refresh();
  }
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-slate-500">{t.descDoctor}</p>
      <p className="text-xs text-slate-400">{t.loginHintDoctor}</p>
      ...
      {step === "list" &&
        (initial.length === 0 ? (
          ...
          <p className="text-xs text-slate-400">{t.limitNoteDoctor}</p>
        ))}
